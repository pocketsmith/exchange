...
describe "Exchange::ExternalAPI::Ecb" do
  ...
  let(:ecb_path) { "http://www.ecb.europa.eu/stats/eurofxref" }
  let(:ecb_90day_url) { "#{ecb_path}/eurofxref-hist-90d.xml" }
  let(:ecb_hist_url) { "#{ecb_path}/eurofxref-hist.xml" }
  let(:ecb_90day_file) { fixture('api_responses/example_ecb_xml_90d.xml') }
  let(:ecb_hist_file) { fixture('api_responses/example_ecb_xml_history.xml') }
  ...
  describe "updating rates" do
    ...
    before(:each) do
      stub_request(:get, ecb_90day_url).to_return(body: ecb_90day_file)
    end
  ...
  describe "historic conversion" do
    ...
    before(:each) do
      stub_request(:get, ecb_hist_url).to_return(body: ecb_hist_file)
    end

    it "should convert and be able to use history" do
      expect(subject.convert(70, :eur, :usd, :at => Time.gm(2011,9,9)).round(2)).
        to eq(91.66)
    end
