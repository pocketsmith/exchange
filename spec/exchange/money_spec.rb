...
describe "Exchange::Money" do
  ...
  let(:ecb_url_path) { "http://www.ecb.europa.eu/stats/eurofxref" }
  let(:test_ecb_url) { "#{ecb_url_path}/eurofxref-hist-90d.xml" }
  let(:test_response) { fixture('api_responses/example_json_api.json') }
  let(:test_ecb_response) { fixture('api_responses/example_ecb_xml_90d.xml') }
    ...
    context "when an api is not reachable" do
      ...
      context "and a fallback api is" do
        it "should use the fallback " do
          ...
          stub_request(:get, test_ecb_url).to_return(body: test_ecb_response)
          ...
          expect(a_request(:get, test_ecb_url)).to have_been_made.times(3)
        end
      end
      ...
      context "and no fallback api is" do
        it "should raise the api error" do
          stub_request(:get, test_url).to_timeout
          stub_request(:get, test_ecb_url).to_timeout

          expect { subject.to(:ch) }.
    ...
    context "with a 'from' currency not provided by the given api" do
      ...
      context "but provided by a fallback api" do
        it "should use the fallback" do
          ...
          expect(subject.api::CURRENCIES).
            to receive(:include?).with(:usd).and_return true
          expect(subject.api::CURRENCIES).
            to receive(:include?).with(:chf).and_return true
    ...
    context "with a 'to' currency not provided by the given api" do
      ...
      context "but not provided by any fallback api" do
        it "should raise the no rate error" do
          allow(Exchange::ExternalAPI::OpenExchangeRates::CURRENCIES).
            to receive(:include?).and_return false
          allow(Exchange::ExternalAPI::Ecb::CURRENCIES).
            to receive(:include?).and_return false
    ...
    describe "+ other" do
      ...
      context "with a big decimal" do
        ...
        it "should be able to add a big decimal below zero" do
          expect((subject + BigDecimal.new("0.45454545")).value.round(8)).
            to eq(BigDecimal.new("0.127454545E1"))
        end

        it "should be able to add a big decimal above zero" do
          expect((subject + BigDecimal.new("12.455")).round(2).value).
            to eq(BigDecimal.new("0.1328E2"))
        end
      end
      ...
      it "should be able to add another currency value" do
        ...
        expect((subject + Exchange::Money.new(30, :chf)).value.round(2)).
          to eq(72.87)
      ...
      it "should raise when currencies get mixed and the configuration does not allow it" do
        ...
        expect { subject + Exchange::Money.new(30, :chf) }.
          to raise_error(Exchange::ImplicitConversionError)
      end
      ...
      context "modifying the base value" do
        ...
        it "should raise when currencies get mixed and the configuration does not allow it" do
          Exchange.configuration.implicit_conversions = false
          expect { @instantiated += Exchange::Money.new(30, :chf) }.
            to raise_error(Exchange::ImplicitConversionError)
          Exchange.configuration.implicit_conversions = true
        end
        ...
        it "should not raise when currencies get mixed and the configuration does not allow if the other currency is the same" do
          ...
          expect { @instantiated += Exchange::Money.new(30, :usd) }.
            not_to raise_error
          Exchange.configuration.implicit_conversions = true
    ...
    describe "- other" do
      ...
      context "with a big decimal" do
        ...
        it "should be able to subtract a big decimal below zero" do
          expect((subject - BigDecimal.new("0.45454545")).value.round(8)).
            to eq(BigDecimal.new("0.182936545455E4"))
        end

        it "should be able to subtract a big decimal above zero" do
          expect((subject - BigDecimal.new("12.455")).round(2).value).
            to eq(BigDecimal.new("0.181737E4"))
        end
      end
      ...
      it "should be able to subtract another currency value" do
        ...
        Exchange.configuration.implicit_conversions = true
        expect((subject - Exchange::Money.new(10, :chf)).value.round(2)).
          to eq(29.04)
        expect((subject - Exchange::Money.new(23.3, :eur)).currency).
          to eq(:usd)
    ...
    describe "* other" do
      ...
      context "with a big decimal" do
        ...
        it "should be able to multiply by a big decimal below zero" do
          expect((subject * BigDecimal.new("0.45454545")).value.round(8)).
            to eq(BigDecimal.new("0.83173635532E3"))
        end

        it "should be able to multiply by a big decimal above zero" do
          expect((subject * BigDecimal.new("12.455")).round(2).value).
            to eq(BigDecimal.new("0.2279041E5"))
        end
      end
      ...
      it "should be able to multiply by another currency value" do
        ...
        expect((subject * Exchange::Money.new(10, :chf)).value.round(1)).
          to eq(438.3)
    ...
    describe "/ other" do
      ...
      context "with a big decimal" do
        ...
        it "should be able to divide by a big decimal below zero" do
          expect((subject / BigDecimal.new("0.45454545")).value.round(8)).
            to eq(BigDecimal.new("0.402560404026E4"))
        end

        it "should be able to divide by a big decimal above zero" do
          expect((subject / BigDecimal.new("12.455")).round(2).value).
            to eq(BigDecimal.new("0.14691E3"))
        end
      end
    ...
    describe "comparison" do
      ...
      context "with identical currencies" do
        it "should be true if the currency and the value is the same" do
          ...
          expect(a_request(:get, test_url)).to_not have_been_made
        end
      ...
      context "with different currencies" do
        ...
        it "should be false if the currency is defined historic and the converted value is different" do
          stub_request(:get, "http://openexchangerates.org/api/historical/2011-01-01.json?app_id=").to_return(body: fixture('api_responses/example_historic_json.json'))
          expect(comp3 == comp6).to eq(false)
          expect(a_request(:get, "http://openexchangerates.org/api/historical/2011-01-01.json?app_id=")).to have_been_made.times(1)
        end
  ...
  describe "to_s" do
    ...
    it "should render only the currency amount if the argument amount is passed" do
      ...
      expect(Exchange::Money.new(23.23252423, :sar).to_s(:amount)).
        to eq("23.23")
    ...
    it "should render only the currency amount and no separators if the argument amount is passed" do
      expect(Exchange::Money.new(2323.232524, :tnd).to_s(:plain)).
        to eq("2323.233")
      expect(Exchange::Money.new(2323.23252423, :sar).to_s(:plain)).
        to eq("2323.23")
      expect(Exchange::Money.new(2323.23252423, :clp).to_s(:plain)).
        to eq("2323")
    ...
    it "should render the currency with a symbol according to ISO 4217 Definitions" do
      expect(Exchange::Money.new(23.232524, :tnd).to_s(:symbol)).
        to eq("TND 23.233")
      expect(Exchange::Money.new(23.23252423, :sar).to_s(:symbol)).
        to eq("﷼23.23")
  ...
  describe "methods via method missing" do
    it "should be able to convert via to_currency to other currencies" do
      ...
      expect(a_request(:get, test_url)).to have_been_made.times(6)
    end

    it "should be able to convert via to_currency to other currencies and use historic data" do
      url = "http://openexchangerates.org/api/historical/2011-10-09.json?app_id="
      ...
      expect(a_request(:get, url)).to have_been_made.times(6)
    end

    it "should use the own time if defined as historic to convert" do
      url = "http://openexchangerates.org/api/historical/2011-01-01.json?app_id="
      stub_request(:get, url).to_return(body: test_response)

      expect(5.in(:eur, :at => Time.gm(2011,1,1)).to(:usd).value).
        to eq(5.in(:eur).to(:usd, :at => Time.gm(2011,1,1)).value)
